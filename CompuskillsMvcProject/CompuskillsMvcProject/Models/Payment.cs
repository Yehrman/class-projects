...
{
    public class Payment //<T>where T:struct
    {
        ...
        private IQueryable<ClientProject> Projects(string user)
        {
        ...
        private IQueryable<CompanyEmployee> CompanyEmployees(string user)
        {
            var employee = db.CompanyEmployees.SingleOrDefault(x => x.EmployeeId == user);
