...
namespace EncryptDecrypt
{
    ...
    {
        SqlConnection Conn = new SqlConnection(@"Data Source =. \SQLEXPRESS; Initial Catalog =EncrpytDecrpyt ; Integrated Security = true");
        //Stteps 1,2,3
        public void ExecuteInsertToDb()
        {
            ...
            if (date == null || DateTime.Today>=date.AddDays(1))
            {
                SetEncryptValues();
                InsertValues();
                Console.WriteLine("Database updated,Fetching new data");
                RetrieveEncryptionValues();
            }
        }
        ...
        //EncryptedValuesRepository repository = new EncryptedValuesRepository();
        private void RetrieveEncryptionValues(DateTime? date=null)
        {
            ...
            while(reader.Read())
            {
               // EncryptedValuesRepository.Id = reader.GetGuid(0);
                ...
                EncryptedValuesRepository.One = reader.GetString(27);
                EncryptedValuesRepository.Two = reader.GetString(28);
                EncryptedValuesRepository.Three = reader.GetString(29);
                EncryptedValuesRepository.Four = reader.GetString(30);
                EncryptedValuesRepository.Five = reader.GetString(31);
                EncryptedValuesRepository.Six = reader.GetString(32);
                EncryptedValuesRepository.Seven = reader.GetString(33);
                EncryptedValuesRepository.Eight = reader.GetString(34);
                EncryptedValuesRepository.Nine = reader.GetString(35);
                EncryptedValuesRepository.Zero = reader.GetString(36);
                EncryptedValuesRepository.DateInserted = reader.GetDateTime(37);
            }
            Conn.Close();
        }
     
        //Insert the encrypted entry (name) into db
        public void InsertEncryptyedVal(string entry)
        {
                 string Entry= EncryptEntry(entry);
            var date = EncryptedValuesRepository.DateInserted;
            Conn.Open();
            SqlCommand command = new SqlCommand($"Insert into EncryptedEntries (Entry,DateAdded) Values ('{Entry}','{date}')",Conn);
            command.ExecuteNonQuery();
            Conn.Close();
        }
        ...
        private void RetrieveEntriesFromDb()
        {
            Conn.Open();
            SqlCommand command = new SqlCommand("Select * from EncryptedEntries ", Conn);
       ...
       public void DecryptData()
        {
            ...
            {
                PredecryptedEntry=  Entries.Rows[i].Field<string>("Entry");
                DateOfInsertion = Entries.Rows[i].Field<DateTime>("DateAdded");
               
                RetrieveEncryptionValues(DateOfInsertion);
                Decrypt(PredecryptedEntry);
               ToLower(DecryptedEntry);
              
                DecryptedNames.Add(DecryptedEntry);
                DecryptedEntry = "";
            }
        }
        public void Dispose()
