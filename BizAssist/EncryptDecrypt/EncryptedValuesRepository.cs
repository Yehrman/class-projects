using System;
using System.Collections.Generic;
using System.Text;

namespace EncryptDecrypt
{
    internal class EncryptedValuesRepository
    {
        // public static Guid Id { get; set; }
        public static string A { get; set; }
        public static string B { get; set; }
        public static string C { get; set; }
        public static string D { get; set; }
        public static string E { get; set; }
        public static string F { get; set; }
        public static string G { get; set; }
        public static string H { get; set; }
        public static string I { get; set; }
        public static string J { get; set; }
        public static string K { get; set; }
        public static string L { get; set; }
        public static string M { get; set; }
        public static string N { get; set; }
        public static string O { get; set; }
        public static string P { get; set; }
        public static string Q { get; set; }
        public static string R { get; set; }
        public static string S { get; set; }
        public static string T { get; set; }
        public static string U { get; set; }
        public static string V { get; set; }
        public static string W { get; set; }
        public static string X { get; set; }
        public static string Y { get; set; }
        public static string Z { get; set; }
        public static string Zero { get; set; }
        public static string One { get; set; }
        public static string Two { get; set; }
        public static string Three { get; set; }
        public static string Four { get; set; }
        public static string Five { get; set; }
        public static string Six { get; set; }
        public static string Seven { get; set; }
        public static string Eight { get; set; }
        public static string Nine { get; set; }
   
        public static string At { get; set; }
        public static string Dot { get; set; }
        public static DateTime DateInserted { get; set; }
    }
}